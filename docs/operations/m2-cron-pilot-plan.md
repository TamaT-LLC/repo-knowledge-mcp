...
| job backlog         | summarize の `backlog`                     | `pending_jobs_monotonically_increasing: false`（`pending_jobs_by_day` の日次系列で確認）。欠測で系列が断絶した場合は `null`（判定不能）になるため自動 go とせず、`backlog_series_gaps` の日付の日次記録と incident を人間が review し、判断根拠を最終 report に記録する |
