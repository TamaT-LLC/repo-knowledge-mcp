...
export async function runPilotDailyRecordCli(
  ...
  try {
    ...
    if (parsed.command === "record") {
      await runRecord(parsed, io);
    } else {
      return await runSummarize(parsed, io);
    }
  } catch (error) {
...
function readRecordOptions(argv: readonly string[]) {
