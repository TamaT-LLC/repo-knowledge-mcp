...
export async function runGoldenBaselineCli(
  ...
  try {
    ...
    assertLiveModePreconditions(parsed, env);
    ...
    if (error instanceof UsageError) {
      ...
      io.stderr.write(`BASELINE_CAPTURE_FAILED: ${message}\n`);
      return 1;
    }
  }
...
function assertLiveModePreconditions(
  parsed: ParsedArguments,
  env: Readonly<NodeJS.ProcessEnv>,
): void {
  if (parsed.mode !== "live") return;
  if (isTruthyEnvironment(env.CI) || isTruthyEnvironment(env.GITHUB_ACTIONS)) {
